...
//     .then((response) => response.json())
//     .then((res) => {
//       const cityName = res.data.city; // 获取城市名称

//       $("#ip_text").html("访问IP：" + res.data.ip + "<br>归属地：" + res.data.desc); // 将城市名称显示在页面上

//       $("#city_text").html(cityName); // 将城市名称显示在页面上

//       fetch(
//         `https://www.mxnzp.com/api/weather/current/${cityName}?app_id=${app_id}&app_secret=${app_secret}`
//       )
//         .then((response) => response.json())
//         .then((res) => {
//           if (res.code == 1) {
//             $("#wea_text").html(res.data.weather); // 天气状况
//             $("#tem_text").html(res.data.temp + "&nbsp;"); // 温度
//             $("#win_text").html(res.data.windDirection + "风"); // 风向
//             $("#win_speed").html(res.data.windPower); // 风力
//           } else {
//             console.error("天气信息获取失败");
//             iziToast.show({
//               timeout: 2000,
...
const getWeather = () => {
  ...
  fetch(`https://www.mxnzp.com/api/ip/self?app_id=${app_id}&app_secret=${app_secret}`)
    ...
    .then((res) => {
      ...
      // 更新 IP 和归属地信息
      $("#ip_text").html("访问IP：" + res.data.ip + "<br>归属地：" + res.data.desc);
      $("#city_text").html(cityName); // 显示城市名称
      ...
      if (cachedData) {
        ...
        if (Date.now() - cache.timestamp < CACHE_EXPIRY) {
          console.log("从缓存中加载天气数据");
          renderWeather(cache.data);
          return;
        }
      }

      // 如果缓存不存在或过期，获取天气数据并更新缓存
      fetch(
        `https://www.mxnzp.com/api/weather/current/${cityName}?app_id=${app_id}&app_secret=${app_secret}`
      )
        .then((response) => response.json())
...
const renderWeather = (data) => {
  $("#wea_text").html(data.weather); // 天气状况
  $("#tem_text").html(data.temp + "&nbsp;"); // 温度
  $("#win_text").html(data.windDirection + "风"); // 风向
  $("#win_speed").html(data.windPower); // 风力
};
...
$("#switchmore").on("click", function () {
  ...
  if (shoemore && $(document).width() >= 990) {
    ...
    $("#change").html("Oops&nbsp;!");
    $("#change1").html("哎呀，这都被你发现了（ 再点击一次可关闭 ）<br>");
  } else {
